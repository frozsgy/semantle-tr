...
cur = con.cursor()
cur.execute(
    """create table if not exists nearby (word text, neighbor text, similarity float, percentile integer)"""
)

con.commit()
cur.execute(
    """create unique index if not exists nearby_words on nearby (word, neighbor)"""
)
con.commit()

cur.execute(
    """create table if not exists similarity_range (word text, top float, top10 float, rest float)"""
)

cur.execute(
    """create unique index if not exists similarity_range_word on similarity_range (word)"""
)
con.commit()
